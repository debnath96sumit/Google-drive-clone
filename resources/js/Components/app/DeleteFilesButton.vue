...
<script setup>
    ...
    function onDeleteCancel(params) {
    ...
    const onDeleteConfirm = ()=>{
        fileDeletedForm.parent_id = page.props.folder.id;
        if (props.deleteAll) {
            fileDeletedForm.all = true;
        }else{
            fileDeletedForm.ids = props.deleteIds;
        }
        fileDeletedForm.delete(route('file.delete'), {
            onSuccess: ()=>{
                showConfirmatonDialog.value = false;
                emit('delete');
                showSuccessNotification('Selected files have been moved to trash');
            }
        });
